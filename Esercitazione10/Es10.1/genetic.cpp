...
Popolazione :: ~Popolazione(){
	...
	vector <Individuo>().swap(pop);
	vector <Individuo>().swap(new_gen);
	vector <double>().swap(yc);
	vector <double>().swap(xc);
...
void Popolazione :: printPop() const{
	...
	for(int j=0; j<I; j++){
		//cout<<"Path: [ ";
		//for(int i=0; i<N-1; i++) cout<<pop[j].get_vec()[i]<<", ";
		//cout<<pop[j].get_vec()[N-1]<< " ] Fitness: "<<get_fitness(j)<<endl;
		pop[j].printInd();
	}
...
/*
	for(int i=0; i<I; i++){
		cout<<get_fitness(i)<<" "<<prob[i]/prob_tot<<endl;
		if(i==49)	cout<<"Metà"<<endl;
	}
*/
...
void Popolazione :: update_gen(){
	...
	if(size_ng>I){
		...
		update_fitness_ng();
		for(int i=0; i<size_ng-I; i++)	new_gen.erase(new_gen.end());	//elimino gli ultimi elementi, cioè con fitness peggiore
