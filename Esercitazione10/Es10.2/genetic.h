...
public:
	...
	void set_comp(int i, int comp){ path[i]=comp;}
	...
	bool mutate(double prob, Random *rnd);
...
public:
	// constructors
	Popolazione();
	Popolazione(int Is, int Ns);
	Popolazione(Individuo ind);
	Popolazione(int Is, Individuo ind);
	Popolazione(const Popolazione &p);
	...
	void set_ind(int i, Individuo ind){pop[i]=ind;}
