...
int main()
{
	...
	double energia=0.0, pressione=0.0;
 
	Input(); //Inizialization
	int nconf = 1;


	for(int istep=1; istep <= n_eq; ++istep)	Move();		//equilibrazione, n_eq= 100 per il solido, 300 per il liquido e 1000 per il gas
	...
	for(int iblk=1; iblk <= nblk; ++iblk) //Simulation
	{
		...
		for(int istep=1; istep <= nstep; ++istep)
		{
			...
			energia = walker[iv]/(double)npart + vtail;
			pressione = rho * temp + (walker[iw] + ptail * (double)npart) / vol;
			risultati<<energia<<"\t"<<pressione<<endl;
...
//Tail corrections for potential energy and pressure
	...
	ReadInput >> n_eq;	//leggo gli step necessari a equilibrare
