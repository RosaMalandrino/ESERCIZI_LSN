...
double variationalWaveFunction(double v)
{
	...
	double mu=0.810563, sigma=0.613417;
...
double variationalWaveFunction_second(double v)
{
	...
	double mu=0.810563, sigma=0.613417;
	double k_minus=pow((v-mu)/sigma,2);
	double k_plus=pow((v+mu)/sigma,2);
	return (k_minus*exp(-0.5*k_minus) + k_plus*exp(-0.5*k_plus) - variationalWaveFunction(v))/pow(sigma, 2);
